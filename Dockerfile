...
# Set user to the app
USER app-user
# setting working dir, docker cretes app dir
WORKDIR /app
...
# same can be done using - ENTRYPOINT [ "npm", "run", "dev", "--", "--host", "--port", "3333"]
CMD [ "npm", "run", "dev", "--", "--host", "--port", "3333"]

################################################################################################
## Build image => docker build -t my-app:1 .
## Run Container => docker run -d -p 3333:3333 --name blue_sky my-app:1